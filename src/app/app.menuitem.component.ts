import {AfterContentInit, AfterViewInit, ChangeDetectorRef, Component, ElementRef, Input, OnDestroy, OnInit, ViewChild, ViewEncapsulation} from '@angular/core';
...
@Component({
    ...
    template: `
        <ng-container>
            <div *ngIf="root" #containerMenu>
                <ng-container *ngIf="item.routerLink!=='' && !item.items then itemLink else itemNoLink"></ng-container>
                <ng-template #itemLink>
                    <a (click)="itemClick($event)" style="display:flex;"
      ...
      <p-overlayPanel #panelSelectMenuDescription [showCloseIcon]="true" appendTo="body">
        <p>{{ 'menu.selectDropdownMenuDescription' | translate }}</p>
      </p-overlayPanel>
...
export class AppMenuitemComponent implements OnInit, OnDestroy, AfterViewInit, AfterContentInit {
    ...
    key!: string;
    @ViewChild('panelSelectMenuDescription', {static: false}) panelSelectMenuDescription: OverlayPanel | undefined = undefined;
    @ViewChild('dropdownMenu', {static: false}) dropdownMenuElm: ElementRef | undefined = undefined;
    ...
    ngAfterViewInit(): void {
      if (this.panelSelectMenuDescription !== undefined && this.dropdownMenuElm !== undefined) {
        this.panelSelectMenuDescription.show(new MouseEvent('click'), this.dropdownMenuElm.nativeElement);
      }
    }
